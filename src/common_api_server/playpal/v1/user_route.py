...
from src.common_api_server.playpal.utilities import get_json_data, json_token_validifier, female_image_filenames, male_image_filenames, decode_json_token, secret_key_jwt, valid_email_regex
...
def playpal_v1_get_user_by_id(user_id):
...
def playpal_v1_user_registration():
    if request.method == 'POST':
        login_response = {'error': 'something went wrong'}
        ...
        if len(filtered_data) != 0:
            ...
            if len(related_bank_account) != 0:
                ...
                login_response = {'authenticationError': "details could not be verified"}
        else:
            login_response = {
...
def playpal_v1_user_verify_login():
    if request.method == 'POST':
...
def playpal_v1_user_verify_username():
    if request.method == 'POST':
        encoded_token = request.json['hash']
